...
const Stats = ({ totalQuestions, totalAnswers, badges, reputation }: StatsProps) => {
  return (
    <div className="mt-10">
      <h4 className="h3-semibold text-dark200_light900">Рейтинг: {reputation}</h4>
      <div className="mt-5 grid grid-cols-1 gap-5 xs:grid-cols-2 md:grid-cols-4">
        <div className="light-border background-light900_dark300 flex flex-wrap items-center justify-evenly gap-4 rounded-md border p-4 shadow-light-300 dark:shadow-dark-200">
          <div className="flex items-center gap-1">
            <p className="body-medium text-dark400_light700">Вопросов:</p>
            <p className="paragraph-semibold text-dark200_light900">{formatAndDivideNumber(totalQuestions)}</p>
          </div>
          <div className="flex items-center gap-1">
            <p className="body-medium text-dark400_light700">Ответов:</p>
            <p className="paragraph-semibold text-dark200_light900">{formatAndDivideNumber(totalAnswers)}</p>
          </div>
        </div>
        <StatsCard imgUrl="/assets/icons/gold-medal.svg" value={badges.GOLD} title="Легенда" />
        <StatsCard imgUrl="/assets/icons/silver-medal.svg" value={badges.SILVER} title="Мастер" />
        <StatsCard imgUrl="/assets/icons/bronze-medal.svg" value={badges.BRONZE} title="Специалист" />
      </div>
    </div>
  );
};
